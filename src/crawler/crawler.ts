...
async function crawlInitiatives() {
    ...
    const mappedInitiatives = jsonInitiatives.map((element : {iniId: string, dataInicioleg: string, iniTitulo: string}) => 
        new Initiative(element.iniId, element.dataInicioleg, element.iniTitulo)
    )
